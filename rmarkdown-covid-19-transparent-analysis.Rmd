...
The number of research articles, reviews, and others:
```{r}
c(research = nrow(filter(opendata, is_research == TRUE)),
  review = nrow(filter(opendata, is_review == TRUE)),
  other = nrow(filter(opendata, is_research == FALSE & is_review == FALSE)))
```
...
## Research papers
First, we subset the data to only include research papers using is_research variable:
```{r}
research <- opendata %>% filter(is_research == TRUE)
```
...
research$jif2020c = ifelse(research$jif2020 < 66.675,"Other", research$journalTitle)
```
